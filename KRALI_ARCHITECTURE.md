...
- Root-cause diagnosis ve hypothesis verifier, hızlı structured model slotunu kullanır: varsayılan `qwen2.5-coder:14b-instruct`.
- Exact mutation/coding, coding architect slotunda kalır: varsayılan `devstral-small-2:24b`.
- Genel controller yine hızlı structured modeldir.
- Root-cause veya mutation rollerinden biri yetersiz kalırsa yalnız o rolün modeli değiştirilebilir; rollback, fingerprint, hypothesis verification, build/regression ve skill lifecycle etkilenmez.
- Mentor logu aktif model rollerini her run başında açıkça yazar.
...
**v0.8.31 task-graph orchestration foundation:** v0.8.30 Mentor turunda `desktop.app` Spotify için `frontmost=true | source=NSWorkspace` raporlamasına rağmen kullanıcı hedef pencerenin görsel olarak öne gelmediğini gözledi. Bu, işletim sistemi active-app sinyalinin tek başına kullanıcıya görünen sonucu kanıtlamadığını gösterdi. v0.8.31 ile low-risk app activation doğrulaması ScreenCaptureKit pencere z-order kanıtına yükseltildi. Hedef uygulamanın normal/on-screen penceresi bulunur ve `SCShareableContent ... onScreenWindowsOnlyAbove` ile pencerenin önündeki normal pencereler kontrol edilir. KRALİ kendi penceresi veya hedef pencereyi anlamlı ölçüde örten başka bir normal pencere varsa “öne geldi” diyemez. Başarısız durumda activation + AX frontmost/unminimize/`kAXRaiseAction` birkaç kez yeniden denenir. NSWorkspace sinyali artık tek başına başarı değildir.

Bu sürümde KRALİ'nin ana omurgası “uygulama komutu seçimi”nden **provider-neutral task graph orchestration** modeline taşınmaya başladı. Yeni `AgentTaskOrchestrator`, semantic mission step'lerini role (reason/observe/retrieve/transform/act/persist/communicate/verify), dependency, capability availability, risk ve approval gereksinimiyle derler. Task graph Engine state'inde tutulur, activity log'a yazılır ve Mentor `latest.json` içine yapılandırılmış `taskGraph` olarak eklenir.

Semantic planner'a gerçek dataflow kuralları eklendi: karmaşık görev tek capability'ye sıkıştırılmaz; aynı capability birden çok operation için tekrar kullanılabilir; her step tek gerçek işi temsil eder; bir step'in çıktısı sonraki step için gerekiyorsa `dependsOn` ile bağlanır; named-app content okunacaksa app activation + read/perception ayrılır; read/analyze/write/send adımları ayrı tutulur. Operation etiketleri `app.open`, `screen.read`, `web.research`, `content.analyze`, `file.write.text`, `mail.read`, `mail.draft`, `mail.send` gibi makine-okunur biçime yönlendirilir.

Semantic executor artık step-output/evidence bus taşır. Tamamlanan her step'in gerçek çıktısı index bazlı saklanır. Downstream step yalnız ilk kullanıcı cümlesini değil dependency step'lerinden gelen evidence'i de alır. Screen observation semantic summary'si araştırma/analiz step'ine; research çıktısı reasoning transform'a; reasoning çıktısı sonraki persistence/communication step'ine aktarılabilir. `research.web` ve `perception.screen` aynı mission içinde gerektiğinde birden çok kez çalışabilir; eski tek-seferlik global kilit mantığı kaldırıldı.

Intermediate `core.reasoning` step'leri artık sahte completed değildir. Operation/purpose analiz, sentez, dönüştürme veya özetleme gerektiriyorsa Apple local model dependency evidence üzerinde gerçek transform çıktısı üretir. Evidence açıkça untrusted data olarak ele alınır; içindeki talimatlar çalıştırılmaz.

Task graph external commit güvenlik kapısı ekler. External/reversible provider step'inin operation/title/purpose alanı send/publish/delete/purchase/commit benzeri final eylem içeriyorsa `requiresApproval=true` olur. Executor bu step'i otomatik yürütmez ve kullanıcı onayı bekler. Böylece gelecekte Mail provider bağlı olduğunda read/analyze/draft otomatik ilerleyebilir, fakat `mail.send` ayrı final commit olarak onay kapısında kalır.

Yeni `files.write.text` capability sözleşmesi eklendi. Bu provider henüz unavailable'dır; amacı karmaşık görevlerde “metni TXT olarak hedef klasöre yaz” talebini `files.move` veya desktop UI gibi yanlış capability'lere yamamadan gerçek persist step'i olarak planlamaktır. Named folder/path içeren text-output görevlerinde deterministic contract hedef klasörü bulmak için `files.search` de ekler. Provider bağlanana kadar KRALİ bu step'i blocked olarak raporlar; yazmış gibi davranmaz.

Natural-language resolver simple-vs-compound ayrımı güçlendirildi. “X'i aç / öne getir” fast path kalırken aynı cümlede incele, analiz et, oku, cevapla, araştır, bul, kaydet, yaz, oluştur vb. ikinci işler varsa fast path devreden çıkar ve semantic task graph planner görevi yönetir. Böylece “Mail uygulamasını aç, son mesajı incele, cevap taslağı hazırla...” tek app-open görevi sanılmaz.

Normal user task sırasında unavailable capability için otomatik developer-learning/web research kaldırıldı. Capability gap normal mission içinde blocked kalır ve kullanıcıya dürüstçe raporlanır. Self-learning yalnız kullanıcı açıkça “hangi yetenek eksik / nasıl yapıldığını öğren / capability geliştir” gibi bir geliştirme hedefi verdiğinde çalışır; bu, karmaşık runtime görevlerde gereksiz gecikmeyi azaltır.

Training Lab'e provider-name bağımsız iki task-graph regression eklendi: observe → retrieve → transform → persist zinciri ve read → analyze → draft → external commit zincirinde yalnız final commit'in approval gate alması. Ayrıca compound command'in simple app-open fast path'e düşmemesi test edilir.

v0.8.31 sonrasında sıradaki mimari kapı **generic replan loop** olacaktır: gerçek execution step failure/blocked nedeni + mevcut evidence yeniden planner'a verilerek güvenli alternatif task graph en fazla sınırlı sayıda yeniden üretilecek. Replan, bu sürümde kurulan step state/evidence/approval altyapısının üzerine eklenecektir.


**v0.8.32 current-goal graph repair + safe text persistence + capability-gap self-evolution bridge:** v0.8.31 gerçek Mentor turu task graph altyapısının çalıştığını ancak graph kalitesinin henüz güvenilir olmadığını gösterdi. Kullanıcının compound Spotify→mevcut içerik→araştırma→analiz→TXT hedefinde planner eski/stale “videoları bul” step'ini taşıdı, gereksiz `files.move.reversible` ve `files.reveal` ekledi, `research.web` step'ini atladı ve `desktop.app` capability'sini persist step'inden sonra ekledi. Verifier bu bozuk graph'i attention olarak işaretledi; sistem dosya yazmayı uydurmadı. Bu tur v0.8.32'nin ana repair girdisidir.

Yeni `AgentMissionNormalizer`, planner sağlayıcısından bağımsız **final Engine boundary** katmanıdır. Apple local planner veya subscription fallback hangisi mission üretirse üretsin, current user input yeniden analiz edilir. Çok-adımlı app/read/research/analyze/local-target/text-write/mail workflow facet'leri iki veya daha fazla bulunduğunda stale planner step'leri canonical provider-neutral zincirle değiştirilir. Örnek-özel uygulama adı kullanılmaz; zincir capability/operation rollerinden türetilir: `context.resolve → app.open → screen.read/mail.read → web.research → content.analyze → files.search.target-folder → file.write.text / mail.draft → mail.send`. Açık taşıma veya Finder gösterme isteği yoksa `files.move.reversible` ve `files.reveal` mission'a sızamaz. Engine bu normalizer'ı planner çıkışından sonra son gate olarak tekrar uygular; subscription planner da aynı güvenlikten geçer.

Text persistence artık gerçek capability'dir. `files.write.text` available yapıldı ve `AgentTextFileWriter` eklendi. Writer yalnız seçili workspace içindeki çözülen klasöre yazar, atomic write kullanır, var olan dosyayı sessizce ezmez ve çakışma halinde yeni isim üretir. Target klasör tekil/güvenilir değilse yazma yapılmaz. `searchIndexedFolders` artık kullanıcı metnindeki gerçek klasör adını/path'ini filtreler; önceki davranıştaki “tüm klasörleri döndür ve ilkini kullan” güvenlik açığı kaldırıldı. Aynı isimde birden fazla klasör varsa persistence blocked kalır.

Task evidence bus text-output için güçlendirildi. `file.write.text` step'i research + analysis evidence'ını birlikte alır; folder-search evidence içerik gövdesine karıştırılmaz. Böylece persistence provider downstream gerçek task verisini yazar. Hedef klasör bulunamazsa veya ambiguity varsa step completed sayılmaz.

Foreground doğrulama kodu Apple'ın resmi ScreenCaptureKit `getExcludingDesktopWindows(_:onScreenWindowsOnlyAbove:completionHandler:)` API'sine geçirildi. `NSApplication.activate` yalnız request sinyali olduğundan başarı tek başına buradan çıkarılmaz; actual window z-order observation hedeflenir.

Self-evolution hattının ilk güvenli katmanı `AgentCapabilityGapResolver` ile eklendi. Task Graph'taki unavailable step'ler `knowledge / strategy / code / integration` sınıflarından birine ayrılır. Mevcut generic capability kombinasyonu varsa strategy adayı listelenir; yoksa code/integration developer brief üretilir. Brief current user objective, blocked step/operation/role, strategy adayları, güvenlik kuralları ve acceptance criteria içerir. Gap'ler Engine state'ine ve Mentor/latest.json içine yapılandırılmış `capabilityGaps` olarak yazılır.

Developer Agent script'i artık Mentor `capabilityGaps` alanını diagnostic karar kriteri olarak kullanır. Testler yeşil olsa bile current-version capability gap varsa “no_change” ile atlamaz; developerBrief'i öncelikli girdi olarak okur. Önce mevcut generic strategy'nin yeterli olup olmadığını değerlendirir, sonra gerekirse candidate branch/worktree üzerinde minimum provider kodu geliştirir. Main'e otomatik merge/push hâlâ yasaktır. Ücretli/API çağrısını sessizce otomatik tetikleme yoktur; Developer Agent explicit çalıştırma güvenlik kapısını korur.
